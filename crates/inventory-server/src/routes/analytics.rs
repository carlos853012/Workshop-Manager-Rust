...
async fn dashboard(
    ...
    let total_sales: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sales WHERE status = 'completed'")
        .fetch_one(&state.pool)
        .await
        .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
    ...
    let pending_repairs: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM repairs WHERE status = 'pending'"
    )
    .fetch_one(&state.pool)
    .await
    .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
    ...
    let completed_repairs: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM repairs WHERE status = 'completed'"
    )
    .fetch_one(&state.pool)
    .await
    .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
...
async fn kpis(State(state): State<AppState>) -> Result<Json<ApiResponse<KpisResponse>>, AppError> {
    let total_suppliers: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM suppliers WHERE status = 'active'")
        .fetch_one(&state.pool)
        .await
        .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
    ...
    let in_progress_repairs: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM repairs WHERE status = 'in_progress'"
    )
    .fetch_one(&state.pool)
    .await
    .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
    ...
    let cancelled_repairs: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM repairs WHERE status = 'cancelled'"
    )
    .fetch_one(&state.pool)
    .await
    .map_err(|e| AppError::Internal(format!("Database error: {}", e)))?;
